...
def _cycle_contexts(d: date_) -> list[dict]:
    ...
    for cycle in focus_cycles:
        cfg = cycle.configuration or db.session.get(Configuration, cycle.configuration_id) or get_configuration_at(cycle.started_at)
        configs.setdefault(cycle.configuration_id, cfg)
    for rest in rest_cycles:
        cfg = rest.configuration or db.session.get(Configuration, rest.configuration_id) or get_configuration_at(rest.started_at)
        configs.setdefault(rest.configuration_id, cfg)
...
def consolidate_daily(d: date_, commit: bool = True, force: bool = False) -> Daily:
    """
    Consolida e persiste o Daily da data informada.

    Apenas grava o que aconteceu e o contexto historico necessario.
    Apos consolidated_at ser definido, o snapshot e imutavel por
    padrao. Passe `force=True` para reconstruir um snapshot ja
    consolidado quando o banco receber dados operacionais retroativos
    (ex.: debug, reconciliação temporal).
    """
    daily = ensure_daily_for_date(d, session_started=False, commit=False)
    if daily.consolidated_at is not None and not force:
        if commit:
            db.session.commit()
        return daily
    ...
    if daily.consolidated_at is None and active_sessions == 0 and d < today():
