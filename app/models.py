...
def _today_default():
    # Import tardio para evitar ciclo de importacao com app.services.clock.
...
class Configuration(db.Model, TimestampMixin):
    ...
    auto_start_focus = db.Column(db.Boolean, nullable=False, default=False)
    auto_start_break = db.Column(db.Boolean, nullable=False, default=False)
    post_cycle_messages_enabled = db.Column(db.Boolean, nullable=False, default=True)

    # --- controle de volume (0 a 100%) ---
    volume_system = db.Column(db.Integer, nullable=False, default=100)
    volume_focus_end = db.Column(db.Integer, nullable=False, default=100)
    volume_rest_end = db.Column(db.Integer, nullable=False, default=100)

    # --- seleção de arquivos de som ---
    sound_focus_end = db.Column(db.String(255), nullable=False, default="/static/sounds/fim_de_foco.mp3")
    sound_conclude_btn = db.Column(db.String(255), nullable=False, default="/static/sounds/do_pos_conclusao.mp3")
    sound_rest_end = db.Column(db.String(255), nullable=False, default="/static/sounds/fim_de_descanso.mp3")
...
class Session(db.Model, TimestampMixin):
    ...
    id = db.Column(db.Integer, primary_key=True)
    date = db.Column(db.Date, nullable=False, default=_today_default, index=True)
...
class FocusCycle(db.Model, TimestampMixin):
    ...
    # Desconto de duração aplicado a este ciclo (só extraordinários com promoção).
    # None = ciclo normal ou extraordinário sem desconto. Valores possíveis: 30, 40, 50.
    # O focus_duration_minutes já reflete o desconto — este campo é apenas para exibição.
...
class ExtraordinaryProgress(db.Model, TimestampMixin):
    ...
    next_discount_pct = db.Column(db.Integer, nullable=True)
...
class TodoListState(db.Model, TimestampMixin):
    ...
    last_completed_date = db.Column(db.Date, nullable=False, default=_today_default)
...
class RewardAchievement(db.Model, TimestampMixin):
    ...
    achieved_date = db.Column(db.Date, nullable=False, default=_today_default)
...
class NotebookEntry(db.Model, TimestampMixin):
    """
    Caderno Diário — registro diário pessoal e retrato.
    """
    ...
    # JSON das linhas do Registro Pessoal (texto, checkbox, bullet, destaque, cor)
    registro_content = db.Column(db.JSON, nullable=False, default=list)

    # Retrato do Dia preenchido pelo usuário
    inicio_estudos = db.Column(db.String(10), nullable=True)
    fim_estudos = db.Column(db.String(10), nullable=True)
    nivel_cansaco = db.Column(db.Integer, nullable=True, default=0)

    # Flag se o painel "Considere isso para o seu dia" foi dispensado/visto hoje
    programacao_dismissed = db.Column(db.Boolean, nullable=False, default=False)

    def as_dict(self) -> dict:
        return {
            "id": self.id,
            "date": self.date.isoformat(),
            "registro_content": self.registro_content or [],
            "inicio_estudos": self.inicio_estudos or "",
            "fim_estudos": self.fim_estudos or "",
            "nivel_cansaco": self.nivel_cansaco or 0,
            "programacao_dismissed": self.programacao_dismissed,
        }

    def __repr__(self):
        return f"<NotebookEntry {self.date}>"


class NotebookSchedule(db.Model, TimestampMixin):
    """
    Programação por Data — itens/demandas agendados para uma data futura específica.
    """
    ...
    copied = db.Column(db.Boolean, nullable=False, default=False)
    ...
    def __repr__(self):
        return f"<NotebookSchedule {self.target_date}: {self.text[:20]}>"


class Protocol(db.Model, TimestampMixin):
    """
    Protocolos Cotidianos — rotinas estruturadas reutilizáveis.
    """
    ...
    avg_duration_minutes = db.Column(db.Integer, nullable=False, default=15)
    recommended_time = db.Column(db.String(60), nullable=True)
    ...
    def as_dict(self) -> dict:
        return {
            ...
            "recommended_time": self.recommended_time or "",
...
class PostCycleNote(db.Model, TimestampMixin):
    """
    Anotações Pós-ciclo — demandas/ideias capturadas durante ou pós-ciclo.
    """
    __tablename__ = "post_cycle_notes"

    id = db.Column(db.Integer, primary_key=True)
    date = db.Column(db.Date, nullable=False, default=_today_default, index=True)
    cycle_number = db.Column(db.Integer, nullable=True)
    content = db.Column(db.Text, nullable=False)
    category = db.Column(db.String(40), nullable=False, default="lembrete")  # ideia, tarefa, lembrete, duvida
    ...
    def __repr__(self):
        return f"<PostCycleNote {self.date} [{self.category}]: {self.content[:20]}>"
