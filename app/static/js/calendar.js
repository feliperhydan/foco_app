(function () {
  ...
  // Carregar heatmap se o painel foi restaurado pelo reload de tema (evento customizado)
  if (wrap) {
    wrap.addEventListener("foco:restore-open", function() {
      if (!loaded) {
        loaded = true;
        loadCalendar(currentMode);
      }
  ...
  // Fallback: se o painel já tem .open ao rodar (ex: outro mecanismo), carregar também
  if (wrap && wrap.classList.contains("open") && !loaded) {
    loaded = true;
    loadCalendar(currentMode);
  }
})();
