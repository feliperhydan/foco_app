...
from app.services import progress_service, rewards_service, daily_service, clock
from app.services.water_service import random_post_cycle_message
...
def complete_focus_cycle(cycle_id: int, title: str | None = None) -> dict:
    ...
    try:
        ...
        # mantém o Daily do dia corrente refletindo o estado mais
        # recente enquanto o dia ainda está em curso (mesmo padrão já
        # usado por water_service/todo_service) — commit único junto
        # com o resto da transação (seção 42).
    ...
    return {
        ...
        "rewards_achieved": [a.title_snapshot for a in achieved],
        "post_cycle_message": random_post_cycle_message(),
    }
